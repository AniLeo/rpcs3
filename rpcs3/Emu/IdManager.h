...
namespace id_manager
{
	...
	{
		static constexpr std::shared_ptr<void>(*load)(cereal_load&) = [](cereal_load& ar) -> std::shared_ptr<void> { return std::make_shared<T>(stx::exact_t<cereal_load&>(ar)); };
	};

	template <typename T>
	struct id_traits_load_func<T, std::void_t<decltype(&T::load)>>
	{
		static constexpr std::shared_ptr<void>(*load)(cereal_load&) = &T::load;
	};
	...
	{
		dummy_construct() {}
		dummy_construct(cereal_load&){}
		void save(cereal_save&) {}
	};

	struct typeinfo
	{
	public:
		std::shared_ptr<void>(*load)(cereal_load&);
		void(*save)(cereal_save&, void*);
		...
		static typeinfo make_typeinfo()
		{
			...
			using C = std::conditional_t<std::is_constructible_v<T, stx::exact_t<cereal_load&>>, T, dummy_construct>;
			...
			if constexpr (std::is_same_v<C, T>)
			{
				info =
				{
					...
					+[](cereal_save& ar, void* obj) { static_cast<C*>(obj)->save(ar); },

...
struct llvm_fmuladd
{
	...
	llvm::Value* eval(llvm::IRBuilder<>* ir) const
	{
		...
		return ir->CreateIntrinsic(force_hw_fma ? llvm::Intrinsic::fma : llvm::Intrinsic::fmuladd, {v1->getType()}, {v1, v2, v3});
	}
	...
	llvm_match_tuple<A1, A2, A3> match(llvm::Value*& value) const
	{
		...
		if (auto i = llvm::dyn_cast_or_null<llvm::CallInst>(value); i && i->getIntrinsicID() == (force_hw_fma ? llvm::Intrinsic::fma : llvm::Intrinsic::fmuladd))
		{
			v1 = i->getOperand(0);
			v2 = i->getOperand(1);
			v3 = i->getOperand(2);
...
public:
	...
	static auto fmuladd(T&& a, U&& b, V&& c, bool force_hw_fma)
	{
		return llvm_fmuladd<T, U, V>{std::forward<T>(a), std::forward<U>(b), std::forward<V>(c), force_hw_fma};
	}

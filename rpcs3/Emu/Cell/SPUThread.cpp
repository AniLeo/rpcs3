...
spu_thread::spu_thread(cereal_load& ar, lv2_spu_group* group)
	: cpu_thread(idm::last_id())
	, group(group)
	, index(ar)
	...
	, spu_tname(make_single<std::string>(ar.operator std::string()))
{
	...
	// Lose reservation with an event if one existed savestate save
	if (ar.operator u8()) set_events(SPU_EVENT_LR);

	{
		u32 vals[4]{};
		const u8 count = ar;
		if (count) ar(cereal::binary_data(vals, sizeof(u32) * count));
...
void spu_thread::save(cereal_save& ar)
{
	ar(index);
	...
	, run_ctrl
	, exit_status.data
	,status_npc.load().status);

	ar(u8{!!raddr});

	{
		u32 vals[4]{};
		u8 count = static_cast<u8>(ch_in_mbox.try_read(vals));
		ar(count); if (count) ar(cereal::binary_data(std::as_const(vals), sizeof(u32) * count));
	}

...
lv2_event_queue::lv2_event_queue(cereal_load& ar) noexcept
	: id(idm::last_id())
	, protocol(ar)
	, type(ar)
	, size(ar)
...
std::shared_ptr<void> lv2_event_queue::load(cereal_load& ar)
{
	auto queue = std::make_shared<lv2_event_queue>(ar);
	return lv2_obj::load(queue->key, queue);
}

void lv2_event_queue::save(cereal_save& ar)
{
	ar(protocol, type, size, name, key, events);
}

void lv2_event_queue::save_ptr(cereal_save& ar, lv2_event_queue* q)
{
	if (!lv2_obj::check(q))
	{
		ar(u32{0});
...
std::shared_ptr<lv2_event_queue> lv2_event_queue::load_ptr(cereal_load& ar, std::shared_ptr<lv2_event_queue>& queue)
{
...
lv2_event_port::lv2_event_port(cereal_load& ar)
	: type(ar)
	, name(ar)
...
void lv2_event_port::save(cereal_save& ar)
{
	ar(type, name);
